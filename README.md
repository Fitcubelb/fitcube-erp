...
- **Checkins tab** — a full log of every real visit across every client (a paid session, an unpaid one, or a used credit), with a search bar, totals for Checkins/Sales/Clients, and a filter sheet just like a point-of-sale checkins log: switch between a flat "Logs" list and colored Day/Month/Year summary cards you tap into, search by name or receipt number, and sort by date, sales, or name. Each entry shows the client, a paid/unpaid/used-a-credit badge, the date and time, and an auto-generated receipt number. Credits that have been given but not yet used don't show up here — only actual visits do.
