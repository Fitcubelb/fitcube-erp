...
- **Record payment** — on any client's page, one screen for Paid, Owes, Use a credit, or Give credits (switch modes with a tap instead of separate flows); see a running balance per client; one tap to mark a session or a package paid.
...
- **Sell session packages / bundles, your own pricing** — ⚙ Settings → Session packages lets you set up whatever bundle pricing you want — "1 session" for $30, a "10-pack" for $450, a "12-pack" for $500, as many as you like, edited or deleted any time. From any client's page, "Record payment" → "Give credits" picks one of those (or a one-off custom name/count/price on the spot), lets you say whether the client has paid yet, and in one step adds that many prepaid session credits to the client. A paid sale books the price as revenue right away — showing up immediately in Accounting and in the Overview revenue/profit numbers; an unpaid one shows up in the client's owed balance until you mark it paid. Every package sold to a client is listed on their page under "Packages sold." Using a credit ("Record payment" → "Use a credit") automatically takes one off the client's remaining balance and records that visit — no manual bookkeeping to keep the count right. Works offline too: credits, sales, and redemptions show up on the client's page right away and sync once you're back online.
...
- **Checkins tab** — a full log of every real visit across every client (a paid session, an unpaid one, or a used credit), matching the exact look and layout of a point-of-sale checkins log: a search bar with a filter button, one totals card (Checkins/Sales/Clients), and either a flat "Logs" list or teal Day/Month/Year summary cards you tap into. Each entry shows the client, their tier (Bronze/VIP/Regular — set per client under "Edit"), "By Fit Cube," the day and month, and a labeled Day/Time/Receipt Number/Amount grid, with an auto-generated receipt number. Search by name or receipt number, sort by date/sales/name. Credits that have been given but not yet used don't show up here — only actual visits do.
